      ...
      >
        <DialogContent className={`sm:max-w-[550px] h-[85vh] bg-surface-elevated border-border text-text-primary shadow-2xl flex flex-col rounded-2xl p-0 border-t-border/50 overflow-hidden antialiased ${isFirstWorkspace ? "[&>button]:hidden" : ""}`}>
          <DialogHeader className="p-8 pb-4 shrink-0 space-y-3">
            ...
            <DialogDescription className="text-text-secondary text-sm leading-relaxed">{t.workspace_setup.description}</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto px-8 scrollbar-hide">
            <form id="ws-form" onSubmit={workspaceForm.handleSubmit(onWorkspaceSubmit)} className="space-y-10 py-6">
              <div className="space-y-3">
                ...
                <Input {...workspaceForm.register("name")} placeholder={t.workspace_setup.name_placeholder} className="bg-background border-border text-text-primary h-12 rounded-xl px-4 font-medium" />
                {workspaceForm.formState.errors.name && (
                  <p className="text-xs text-danger font-medium flex items-center gap-1"><AlertCircle size={12}/> {workspaceForm.formState.errors.name.message}</p>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-text-secondary font-medium">{t.workspace_setup.core_time_guide}</p>
                  </div>
                  <Button 
                    type="button" 
                    variant="ghost" 
                    ...
                    className="h-8 text-xs font-bold text-text-secondary hover:text-text-primary hover:bg-border transition-all rounded-lg uppercase tracking-wider"
                  >
                    {t.workspace_setup.core_time_reset}
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <Input type="time" {...workspaceForm.register("coreTimeStart")} className="bg-background border-border text-text-primary h-12 rounded-xl px-4 font-medium [color-scheme:dark]" />
                  <Input type="time" {...workspaceForm.register("coreTimeEnd")} className="bg-background border-border text-text-primary h-12 rounded-xl px-4 font-medium [color-scheme:dark]" />
                </div>
                {workspaceForm.formState.errors.coreTimeEnd && (
                  <p className="text-xs text-danger font-medium flex items-center gap-1"><AlertCircle size={12}/> {workspaceForm.formState.errors.coreTimeEnd.message}</p>
                )}
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-text-secondary font-medium">{t.workspace_setup.unplugged_guide}</p>
                  </div>
                  <Button 
                    type="button" 
                    variant="outline" 
                    ...
                    className="border-border bg-background hover:bg-border text-text-secondary font-bold rounded-lg h-9"
                  >
                ...
                <div className="space-y-4 pb-2">
                  {fields.map((field, index) => (
                    <div key={field.id} className="p-5 bg-background/60 border border-border rounded-2xl space-y-5 relative animate-in slide-in-from-top-4 duration-300">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-text-secondary tracking-widest uppercase">Block #{index + 1}</span>
                        <button type="button" onClick={() => remove(index)} className="text-text-muted hover:text-danger transition-colors active:scale-75">
                          <X size={16} />
                        </button>
                      </div>
                      <div className="space-y-2">
                          <Input {...workspaceForm.register(`unpluggedTimes.${index}.label` as const)} placeholder={t.workspace_setup.unplugged_label_placeholder} className="bg-surface-elevated border-border h-11 rounded-xl px-4 font-medium" />
                          {workspaceForm.formState.errors.unpluggedTimes?.[index]?.label && (
                              <p className="text-xs text-danger font-medium pl-1">{workspaceForm.formState.errors.unpluggedTimes[index]?.label?.message}</p>
                          )}
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <Input type="time" {...workspaceForm.register(`unpluggedTimes.${index}.startTime` as const)} className="bg-surface-elevated border-border h-11 rounded-xl font-medium [color-scheme:dark]" />
                        <div className="space-y-1">
                          <Input type="time" {...workspaceForm.register(`unpluggedTimes.${index}.endTime` as const)} className="bg-surface-elevated border-border h-11 rounded-xl font-medium [color-scheme:dark]" />
                          {workspaceForm.formState.errors.unpluggedTimes?.[index]?.endTime && (
                              <p className="text-xs text-danger font-medium pl-1">{workspaceForm.formState.errors.unpluggedTimes[index]?.endTime?.message}</p>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  className="w-full min-h-[140px] bg-background border-border rounded-2xl p-5 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-white/10 placeholder:text-text-muted font-medium leading-relaxed shadow-inner"
                />
              </div>
            </form>
          </div>
          ...
          <DialogFooter className="p-8 pt-4 pb-8 border-t border-border bg-surface-elevated shrink-0">
            <Button type="submit" form="ws-form" className="w-full bg-text-primary text-background hover:bg-zinc-200 font-bold h-12 rounded-xl text-md transition-all shadow-xl shadow-black/20 active:scale-95">
