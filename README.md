...
태스크를 끝내면 실제 소요 시간과 짧은 회고 메모를 남깁니다.
...
하루가 끝날 때 회고 버튼을 누르면, `DONE` 상태로 쌓인 태스크들의 계획 메모와 완료 메모를 모아 AI가 일별 회고 문서를 정성껏 작성해 줍니다. 이직을 준비할 때, 연봉을 협상할 때, 혹은 경력기술서를 쓸 때. "내가 거기서 뭘 했더라" 하고 기억을 더듬으며 고생하지 않도록 돕고 싶었습니다.
...
지금은 회고 중심의 기능이 주를 이루고 있지만, 궁극적인 목표는 하나입니다.
...
```text
Inbox → NOW / WILL → DONE → Retrospective
            ↑
...
| `DONE` | 완료. 실제 소요 시간과 회고 메모가 기록되어 있습니다. |
