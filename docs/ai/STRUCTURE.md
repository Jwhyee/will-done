...
- **[Trigger]**: 
  ...
  - **제안**: 현재 요일에 해당하는 고정 업무 템플릿이 존재하고, 아직 오늘 타임라인/인박스에 등록되지 않은 경우 `WorkspaceView` 상단에 노출.
- **[Action]**:
  - **등록**: `RecurringTaskForm`을 통해 제목, 예상 시간, 반복 요일, 계획 메모를 설정하여 저장. `TimePicker` 컴포넌트를 사용하여 직관적인 시간 설정을 지원하며, 중첩 폼(Nested Form) 문제를 해결하여 페이지 새로고침 없이 안정적으로 동작합니다.
  - **추가**: 제안 바의 루틴 칩 클릭 시 해당 템플릿 정보를 복사(Hard Copy)하여 타임라인에 즉시 태스크로 등록.
- **[UI Feedback]**: 
