...
## 1. Left Sidebar (Navigation & Inbox)
*Layout: Fixed width (e.g., 256px), docked to the left. Collapsible via a Hamburger menu icon with a smooth horizontal transition.*

- **Top (Workspace Switcher & Archive)**:
    - A dropdown selector showing the current Workspace Name.
    - A Calendar component. Selecting a week/month here reveals a **[✨ Generate Weekly/Monthly Retrospective]** button.
- **Middle (Inbox / Unscheduled Queue)**:
    - A scrollable list of Task Cards.
    - Cards MUST have a drag-handle icon indicating they can be dragged and dropped onto the Main Timeline.
- **Bottom (Settings)**:
...
*Layout: Takes the remaining horizontal space. Divided into Header and Timeline Board.*
...
### C. AI Retrospective Viewer Modal
- **Layout**: A wide, spacious modal focusing on readability.
- **Content**: A rich Markdown viewer displaying the AI-generated summary.
